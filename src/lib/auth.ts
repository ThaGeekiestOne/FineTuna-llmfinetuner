import { authenticatedFetch, clearSupabaseSession, saveSupabaseSession } from './authTransport'
...
export async function getAuthSession(): Promise<AuthUser | null> {
  ...
  if (!payload.authenticated) clearSupabaseSession()
...
export async function completeOAuthSignIn(input: { accessToken: string; refreshToken: string }): Promise<AuthUser> {
  ...
  saveSupabaseSession(input)
  return payload.user
}

export async function signOut(): Promise<void> {
  const response = await authenticatedFetch('/api/auth/session', { method: 'DELETE' })
